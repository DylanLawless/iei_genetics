...
#   min_prob = colDef(name = "Min"),
#   median_prob = colDef(name = "Median"),
#   max_prob = colDef(name = "Max"),
...
#       vals <- c(df$min_prob[index], df$median_prob[index], df$max_prob[index])
...
# library(scales)
# 
# # Create a new column gene_number preserving the order of genename
# gene_order <- varRisEst_gene_scored %>%
#   distinct(genename) %>%
#   mutate(gene_number = row_number())
# 
# varRisEst_gene_scored <- varRisEst_gene_scored %>%
#   left_join(gene_order, by = "genename")
# 
# p2 <- ggplot(varRisEst_gene_scored, aes(x = count, y = gene_number, fill = score, group = score_factor)) +
#   geom_bar(stat = "identity", position = "fill") +
#   scale_fill_gradient2(low = "navy", mid = "lightblue", high = "red", midpoint = 0) +
#   scale_x_continuous(labels = percent_format()) +
#   # scale_y_continuous(breaks = gene_order$gene_number[seq(1, nrow(gene_order), 50)]) +
#   labs(
#     x = "Percentage of total count per gene",
#     y = "Gene number",
#     fill = "Score"
#   )
# 
# ggplot(varRisEst_gene_scored, aes(x = count, y = genename, fill = score)) +
#   geom_bar(stat = "identity", position = "fill") +
#   scale_fill_gradient2(low = "navy", mid = "lightblue", high = "red", midpoint = 0) +
#   scale_x_continuous(labels = percent_format()) +
#   scale_y_discrete(breaks = function(x) x[seq(1, length(x), 50)]) +
#   labs(
#     x = "Percentage of total count per gene",
#     y = "Gene name",
#     fill = "Score"
...
# 
# 
# # Create an artificial y axis: assign a unique row number to each gene
# gene_order <- varRisEst_gene_scored %>%
#   distinct(genename) %>%
#   mutate(gene_number = row_number())
# 
...
#   left_join(gene_order, by = "genename")
# 
# ggplot(varRisEst_gene_scored, aes(x = count, y = gene_number, fill = score)) +
#   geom_bar(stat = "identity", position = "fill") +
#   scale_fill_gradient2(low = "navy", mid = "lightblue", high = "red", midpoint = 0) +
#   scale_x_continuous(labels = percent_format()) +
#   scale_y_continuous(breaks = seq(1, max(varRisEst_gene_scored$gene_number), 50)) +
#   labs(
#     x = "Percentage of total count per gene",
#     y = "Gene number",
#     fill = "Score"
...
# # library(patchwork)
# # 
# # p1 <- ggplot(gene_summary, aes(x = reorder(genename, -Pathogenic), y = Pathogenic)) +
# #   geom_col(fill = "red") +
# #   labs(x = "Gene name", y = "Pathogenic count", title = "Pathogenic variants per gene") +
# #   theme_minimal() +
# #   theme(axis.text.x = element_text(angle = 45, hjust = 1))
# # 
# # p2 <- ggplot(gene_summary, aes(x = reorder(genename, -Other), y = Other)) +
# #   geom_col(fill = "blue") +
# #   labs(x = "Gene name", y = "Other count", title = "Other variants per gene") +
# #   theme_minimal() +
# #   theme(axis.text.x = element_text(angle = 45, hjust = 1))
# # 
# # p1 + p2
...
# # library(dplyr)
# # 
# # # Order clinvar_clnsig levels by their average score
# # clinvar_levels <- varRisEst_gene_scored %>%
# #   group_by(clinvar_clnsig) %>%
# #   summarise(avg_score = mean(score)) %>%
# #   arrange(avg_score) %>%
# #   pull(clinvar_clnsig)
# # 
# # # Convert clinvar_clnsig to a factor with the ordered levels
# # varRisEst_gene_scored <- varRisEst_gene_scored %>%
# #   mutate(clinvar_clnsig = factor(clinvar_clnsig, levels = clinvar_levels))
# 
# 
# 
# 
...
# library(dplyr)
# 
# # First plot: Average rank vs count of variants per gene
# p1 <- ggplot(gene_summary, aes(x = avg_rank, y = total_variants)) +
...
# print(p1)
# 
# # Second plot: Normalise average rank (zero centred) and plot
...
#   scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
#   labs(
#     x = "Normalised Average Rank Score (zero centred)",
#     y = "Count of Variants per Gene",
#     fill = "Norm Avg Rank"
#   ) +
#   theme_bw()
...
# patch1 <- p1 / p2
# 
# # Histogram of avg_rank: Number of genes per average rank bin
# p1_hist <- ggplot(gene_summary, aes(x = avg_rank)) +
#   geom_histogram(bins = 30, colour = "black", aes(fill = ..x..)) +
#   scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
#   labs(
#     x = "Average Rank Score",
#     y = "Number of Genes",
...
#   theme_bw()
# 
# # Histogram of norm_avg_rank: Number of genes per normalised rank bin
